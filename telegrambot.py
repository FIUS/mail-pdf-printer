...
def printFile(userID, file, lprCmd=None):
    ...
    if lprCmd is None:
        if users[str(userID)]:
            ...
            lprCommand = 'lpr -P fius-printer -o sides=two-sided-long-edge '+file
            subprocess.run(shlex.split(lprCommand))
        else:
            #os.system('lpr -o sides=one-sided '+file)
            lprCommand = 'lpr -P fius-printer -o sides=one-sided '+file
            subprocess.run(shlex.split(lprCommand))
    else:
...
def start(update, context):
    ...
    context.bot.send_message(
        chat_id=update.effective_chat.id, text="An authorization request has been sent")
    button_list = [
        InlineKeyboardButton(
            "Ja", callback_data=str(update.effective_chat.id)),
        InlineKeyboardButton("Nein", callback_data="none")]
    reply_markup = InlineKeyboardMarkup(build_menu(button_list, n_cols=2))
    context.bot.send_message(chat_id=config.adminChatID, text='Darf ' +
                             update.effective_chat.first_name+' admin sein?', reply_markup=reply_markup)
...
def admin_handle(update, cbContext):
    ...
    if button != "none":
        ...
        cbContext.bot.send_message(chat_id=int(
            button), text="You can now print")
    else:
...
def photo(update, cbContext):
    ...
    if isAdmin:
        ...
        f = newFile.download()
        f = str(f)
        cbContext.bot.sendMessage(
            chat_id=update.message.chat_id, text="Picture will be printed")

        printFile(update.message.chat_id, f,
                  lprCmd=update.effective_message.caption)
...
def document(update, cbContext):
    if checkAdmin(update.effective_chat.id):
        ...
        if f.endswith(".pdf"):
            cbContext.bot.sendMessage(
                chat_id=update.message.chat_id, text="File will be printed")

            printFile(update.message.chat_id,f,lprCmd=update.effective_message.caption)
        else:
            cbContext.bot.sendMessage(
                chat_id=update.message.chat_id, text="Can only print pdf")
